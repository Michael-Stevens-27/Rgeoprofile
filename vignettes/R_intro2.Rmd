...
As we can see we provide the matrix function with `data`, the number of rows and columns (`nrow` and `ncol`) and if we should fill the matrix by row or by column. To access a particular value in a matrix, we once again use the square brackets with a refference to the corresponding row and column of that value.  
...
images - matrix_help.png

It may cumbersome to write out all of the arguments in a function, so it is possible simply to write the value of the argument in thefunction leading to the same result:
