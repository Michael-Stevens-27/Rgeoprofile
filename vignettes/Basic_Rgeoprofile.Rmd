...
Bedore we start using the model we will make sure our data are in the correct format using the `geoData()` and `geoDataSource()` functions.
...
Here the colour scheme moves from yellow to purple, where yellow indicates areas we believe might conatin a source location. The purple on the other hand illustrates areas we believe are less likely to contain a source location. This does not tell us too much. The `geoPlotLeaflet()` function can be customised to display more information on the map. Let's add sources and crimes onto the map, as well as a legend that describes the colours we are seeing. 
